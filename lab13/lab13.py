...
def data_base_check(file_name):
    with open(file_name, "r") as file:
        for line in file:
            ...
            if len(line_split[0]) > 22 or len(line_split[1]) > 12 or len(line_split[2]) > 7 or len(line_split[3]) > 43:
                print("[!]Error. File is not a data_base.")
                return 0
...
def initialize(file_name):
    try:
        ...
        print("[!]Error. File can't be created or opened.")
        return 0
    ...
    while True:
        ...
        if initializing_state in ["Yes", "Y", "yes", "y"]:
            ...
            print("[!]Error. Wrong confirming message. Enter `yes` or `no`.")
...
def add_record_file(file_name):
    ...
    while len(license_plate) >= 12 or ',' in license_plate:
        print("[!]Error. Length of the licence plate in the record should be less then 13."
              "\n There must be no `,` in the record.")
        license_plate = input("Enter license plate of the car: ")

    year = input("Enter year of the manufacture: ")
    while len(year) >= 4 or ',' in year:
        print("[!]Error. Length of the year of the manufacture: in the record should be less then 8."
              "\n There must be no `,` in the record.")
        year = input("Enter year of the manufacture: ")
...
def search_by_1(file_name):
    ...
    if len(found_lines) > 0:
        ...
        print("There is no such lines in the data_base.")
...
def search_by_2(file_name):
    ...
    command1 = input_errors_checking("Enter number of the column to search by: ", 1, 4)
    parameter1 = input("Enter item to search by: ")

    command2 = input_errors_checking("\nEnter another number of the column to search by: ", 1, 4)
    ...
    for line in file:
        line_split = line.split(',')
        if line_split[command1 - 1] == parameter1 and line_split[command2 - 1] == parameter2:
            found_lines.append("│ {:22s} │ {:12s} │ {:7s} │ {:43s} │".format(line_split[0], line_split[1],
                                                                             line_split[2], line_split[3]))
    file.close()
...
def commands_block():
    ...
    while command != 7:
        ...
        if command == 2:
            ...
            search_by_1(file_name)

        elif command == 6:
            search_by_2(file_name)
