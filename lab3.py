...
while True:
    ...
    # Counting sides of the triangle
...
# Count a distance from the point to the triangle vertices
side1_additional = ((x1 - x) ** 2 + (y1 - y) ** 2) ** 0.5
side2_additional = ((x2 - x) ** 2 + (y2 - y) ** 2) ** 0.5
side3_additional = ((x3 - x) ** 2 + (y3 - y) ** 2) ** 0.5

# Count the perimeter, area and height of each created triangle
...
# Count the perimeter, area and height of the given triangle
p = (side1 + side2 + side3) / 2
s = (p * (p - side1) * (p - side2) * (p - side3)) ** 0.5

# Check if the point is in the area of the triangle
if abs(s_t3 + s_t2 + s_t1 - s) <= eps:
    print("\nThis point is in the area of the triangle. The smallest destination is {:.5g}.".format(min(d1, d2, d3)))
else:
