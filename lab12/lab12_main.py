...
given_text = ["Every inch of wall space is covered by a bookcase. Each bookcase has 6.9 - 0.9 shelves, 5",
              "+6going almost to the ceiling. Some bookshelves are stacked to the brim with hardback books:",
              "science, maths, history, and everything else. Other shelves have two layers",
              "of paperback science fiction, with the back layer of books propped up on old tissue10+",
...
def commands_block():
    ...
    while command != 8:
        ...
        if command == 1:
            alignment_left(given_text)

        elif command == 2:
            alignment_right(given_text)

        elif command == 3:
            alignment_width(given_text)

        elif command == 4:
            word_to_delete = str(input("Enter word to delete: "))
            delete_word(given_text, word_to_delete)

        elif command == 5:
            word_to_replace = str(input("Enter word to replace: "))
            word_replacing = str(input("Enter replacing word: "))
            replace_word(given_text, word_to_replace, word_replacing)

        elif command == 6:
            compute(given_text)

        elif command == 7:
            delete_longest_sentence(given_text)
