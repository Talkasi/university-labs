...
def find_intervals(start, end, step, function):
    ...
    while x_0 < end:
        ...
        if evaluate(x_1, function) == "Error" or evaluate(x_0, function) == "Error":
            return "Error"
...
def clarification(function, start, end, n_max, eps, n):
    ...
    if evaluate(start, function) == 0:
        return [n + 1, interval, format(start, "<9.10"), format(evaluate(start, function), "<.0e"), n_cur, 0]
    if evaluate(end, function) == 0:
        return [n + 1, interval, format(end, "<9.10"), format(evaluate(end, function), "<.0e"), n_cur, 0]
    ...
    return [n + 1, interval, format(x_mid, "<9.10"), format(evaluate(x_mid, function), "<.0e"), n_cur, 0]
