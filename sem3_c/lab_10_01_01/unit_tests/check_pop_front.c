...
START_TEST(empty_list_remains)
{
    ...
    void *expected = &test[0];
    void *result = pop_front(&head_main);
...
START_TEST(char_data_pop)
{
    ...
    size_t test_n = sizeof(test) / sizeof(test[0]);
    init_list_from_array(&head_main, test, test_n, sizeof(char));
    init_list_from_array(&head_cp, &test[1], test_n - 1, sizeof(char));
...
START_TEST(int_data_pop)
{
    ...
    init_list_from_array(&head_cp, &test[1], test_n - 1, sizeof(int));
...
START_TEST(double_data_pop)
{
    node_t *head_main = NULL;
    node_t *head_cp = NULL;
    double test[] = {1.2314, 0.1242, 334.34};
    size_t test_n = sizeof(test) / sizeof(test[0]);
    init_list_from_array(&head_main, test, test_n, sizeof(double));
    init_list_from_array(&head_cp, &test[1], test_n - 1, sizeof(double));
...
START_TEST(car_struct_data_pop)
{
    ...
    init_list_from_array(&head_cp, &test[1], test_n - 1, sizeof(car_t));
...
Suite *pop_front_suite()
{
    ...
    s = suite_create("Pop front suite");
    tc_core = tcase_create("Pop front");
