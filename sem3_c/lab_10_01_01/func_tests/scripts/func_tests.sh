...
for args_file in func_tests/data/pos_*_args.txt; do
    ...
    out_file="func_tests/data/pos_""$test_number""_out_file.txt"
    result="func_tests/data/pos_""$test_number""_result.txt"

    if func_tests/scripts/pos_case.sh "$out_file" "$result" "$args_file" "$1"; then
...
for args_file in func_tests/data/neg_*_args.txt; do
    ...
    if func_tests/scripts/neg_case.sh "$in_file" "$args_file" "$1"; then
        echo -e "Test ""$test_number"": ${GREEN}PASSED${NC}"
    else
        n_failed=$((n_failed + 1))
        echo -e "Test ""$test_number"": ${RED}FAILED${NC}"
